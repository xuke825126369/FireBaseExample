...
function CollectSplashUI:createAndShow2()
    ...
    if CookingFeverUnloadedUI.bCollectNotify then
        ...
        self.bCanHide = true
        self.textCoinLeft.text = string.format("COIN LEFT:  %d", CookingFeverDataHandler.data.nAction)
...
function CollectSplashUI:Init()
    ...
    if GameConfig.IS_GREATER_169 then
        self.popController.adapterContainer.localScale = Unity.Vector3.one * 0.8
    end
    ...
    btnNotify.onClick:AddListener(function()
        if self.bCanHide then
            GlobalAudioHandler:PlayBtnSound()
            CookingFeverUnloadedUI.bCollectNotify = not CookingFeverUnloadedUI.bCollectNotify
            goCheckMark:SetActive(CookingFeverUnloadedUI.bCollectNotify)
        end
    end)
    self.textCoinLeft = self.transform:FindDeepChild("textCoinLeft"):GetComponent(typeof(TextMeshProUGUI))
end
