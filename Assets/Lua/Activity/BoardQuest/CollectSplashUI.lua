...
function CollectSplashUI:createAndShow2()
    ...
    if BoardQuestUnloadedUI.bCollectNotify then
        ...
        self.bCanHide = true
        self.textAction.text = string.format("DICES LEFT:  %d", BoardQuestDataHandler.data.nAction)
...
function CollectSplashUI:Init()
    ...
    btnNotify.onClick:AddListener(function()
        GlobalAudioHandler:PlayBtnSound()
        if self.bCanHide then
            BoardQuestUnloadedUI.bCollectNotify = not BoardQuestUnloadedUI.bCollectNotify
            goCheckMark:SetActive(BoardQuestUnloadedUI.bCollectNotify)
        end
    end)
