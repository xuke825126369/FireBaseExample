...
function OceanLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
...
function OceanLevelUI:initLevelUI()
    self:InitVariable()
    OceanFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/Ocean/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/Ocean/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

    self.mBonusGameUI = require "Lua/ThemeVideo/Ocean/BonusGameUI"
    self.mBonusGameUI:Init()

    self.mBonusGameBeginUI = require "Lua/ThemeVideo/Ocean/BonusGameBeginSplashUI"
    self.mBonusGameBeginUI:Init()

    self.mBonusGameFinishUI = require "Lua/ThemeVideo/Ocean/BonusGameFinishSplashUI"
    self.mBonusGameFinishUI:Init()
    
    self.mJackPotUI = require "Lua/ThemeVideo/Ocean/JackPotUI"
    self.mJackPotUI:Init()

end     

function OceanLevelUI:Update()
    self.mJackPotUI:Update()
end 

function OceanLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(OceanFunc)
end

function OceanLevelUI:CancelLeanTween()
...
function OceanLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function OceanLevelUI:FindSymbolElement(goSymbol, strKey)
    local tablePoolKey = {"gem1", "gem2", "gem3"}
...
function OceanLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if OceanSymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not OceanFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    OceanFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function OceanLevelUI:isScatterPossibleUseful(nReelId)
    ...
    if SlotsGameLua.m_GameResult:InFreeSpin() then
        return nScatterCount + SlotsGameLua.m_nReelCount - nReelId >= 1
    else
        return nScatterCount + SlotsGameLua.m_nReelCount - nReelId >= 3
    end
end

function OceanLevelUI:isNeedWaitingFreeSpin(nReelId)
...
function OceanLevelUI:CheckRecoverInfo()
    self.tableCachePos = {}
    ...
    OceanLevelUI:getDBBonsuGame()
    if OceanFunc.bInBonusGame then
        SlotsGameLua.m_bSplashFlags[SplashType.CustomWindow] = true
        SlotsGameLua.m_bInResult = true
        SlotsGameLua.m_bInSplashShow = true
        SlotsGameLua.m_nSplashActive = SplashType.CustomWindow

        OceanFunc.tableNowbGetJackPot = {}
        OceanFunc.tableNowJackPotMoneyCount = {}
        for i = 1, 4 do
            OceanFunc.tableNowbGetJackPot[i] = false
            OceanFunc.tableNowJackPotMoneyCount[i] = OceanLevelUI.mJackPotUI:GetTotalJackPotValue(i)
        end

        self.mBonusGameUI:Show(true)
    end

end

-- 显示Scatter 特效 -- 统一使用播放spine动画的接口: PlayActiveAnimation(enumType, true, 1.0)
function OceanLevelUI:ShowScatterBonusEffect()
...
function OceanLevelUI:handleFreeSpinBegin()
...
function OceanLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function OceanLevelUI:ShowCustomWindow()
    AudioHandler:PlayThemeSound("bonusTrigger")
    ...
    for x = 0, SlotsGameLua.m_nReelCount - 1 do
        for y = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if OceanSymbol:isBonusSymbol(nSymbolId) then
                local goSymbol = reel.m_listGoSymbol[y]
                SymbolObjectPool.m_mapSpinEffect[goSymbol]:PlayAnimation("animation", 0.0, true)
            end
        end
    end

    LeanTween.delayedCall(2.0, function()
        self.mBonusGameBeginUI:Show()
    end)    

end

function OceanLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function OceanLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function OceanLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function OceanLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function OceanLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function OceanLevelUI:CacheSymbolElementLayer(goSymbol)
...
function OceanLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function OceanLevelUI:GetEffectByEffectPool(strKey)
...
function OceanLevelUI:RecycleEffectToEffectPool(UsedObj)
...
-------------------------------------------- ReSpin 记录 -----------------------------------------------
function OceanLevelUI:setDBBonsuGame()
    local strLevelName = ThemeLoader.themeKey
	if LevelDataHandler.m_Data.mThemeData == nil then
		LevelDataHandler.m_Data.mThemeData = {}
    end 

    LevelDataHandler.m_Data.mThemeData.tableClickedIndex = self.mBonusGameUI.tableClickedIndex
    LevelDataHandler.m_Data.mThemeData.bInBonusGame = OceanFunc.bInBonusGame
    LevelDataHandler.m_Data.mThemeData.tableBonusSelectIndex = OceanFunc.tableBonusSelectIndex

    setmetatable(LevelDataHandler.m_Data.mThemeData.tableClickedIndex, {__jsontype = "array"})
    setmetatable(LevelDataHandler.m_Data.mThemeData.tableBonusSelectIndex, {__jsontype = "array"})
    LevelDataHandler:persistentData()
end

function OceanLevelUI:getDBBonsuGame()
	if LevelDataHandler.m_Data.mThemeData == nil then
		return
	end

	if LevelDataHandler.m_Data.mThemeData.bInBonusGame == nil then
		return
	end         

    OceanFunc.bInBonusGame = LevelDataHandler.m_Data.mThemeData.bInBonusGame
    OceanFunc.tableBonusSelectIndex = LevelDataHandler.m_Data.mThemeData.tableBonusSelectIndex
    self.mBonusGameUI.tableClickedIndex = LevelDataHandler.m_Data.mThemeData.tableClickedIndex
end
