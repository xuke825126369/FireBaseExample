...
function JackPotUI:Init()
	...
	self.tableJackPotText = {}
	self.tableJackPotText[1] = self.m_transform:FindDeepChild("MiniValue/miniTextValue"):GetComponent(typeof(TextMeshPro))
	self.tableJackPotText[2] = self.m_transform:FindDeepChild("MinorValue/minorTextValue"):GetComponent(typeof(TextMeshPro))
	self.tableJackPotText[3] = self.m_transform:FindDeepChild("MajorValue/majorTextValue"):GetComponent(typeof(TextMeshPro))
	self.tableJackPotText[4] = self.m_transform:FindDeepChild("GrandValue/grandTextValue"):GetComponent(typeof(TextMeshPro))
...
function JackPotUI:addJackPotValue()
	...
	local nJackPotCount = #OceanConfig.TABLE_JACKPOT_ADDCOEF
	for i = 1, nJackPotCount do
		self.tableJackPotAddSumMoneyCount[i] = self.tableJackPotAddSumMoneyCount[i] + nTotalBet * OceanConfig.TABLE_JACKPOT_ADDCOEF[i]
	end

	if not OceanFunc.m_bSimulationFlag then
		for i = 1, nJackPotCount do
			local fValue = self:GetTotalJackPotValue(i)
...
function JackPotUI:GetTotalJackPotValue(i)
	return SceneSlotGame.m_nTotalBet * OceanConfig.TABLE_JACKPOT_MIN_MULTUILE[i] + self.tableJackPotAddSumMoneyCount[i]
end
