...
function DeckResultPayLines:PlayHitLineEffect2(effectPos, nEffectKey, x, y)
	...
	if not bHasHitEffectFlag then
		local strEffectName = "lztukuaiEffect"
		if SlotsGameLua.m_GameResult:InFreeSpin() then
			strEffectName = "lztukuai2Effect"
		end
		local effectObj = RedHatLevelUI:GetEffectByEffectPool(strEffectName)
		effectObj.transform:SetParent(RedHatLevelUI.m_transform, false)
...
function DeckResultPayLines:checkNeedReusedHitLineEffect()
	...
	for i=1, cnt do
		...
		RedHatLevelUI:RecycleEffectToEffectPool(effectObj)
...
function DeckResultPayLines:MatchLineHide()
	...
	for k, v in pairs(self.m_mapHitLineEffect) do
		RedHatLevelUI:RecycleEffectToEffectPool(v)
	end
	self.m_mapHitLineEffect = {}
...
function DeckResultPayLines:GetGoSymbol(nReelId, nRowIndex)
	...
	if SlotsGameLua.m_GameResult:InFreeSpin() and SlotsGameLua.m_GameResult.m_nFreeSpinCount > 0 then
		if nSymbolId == SlotsGameLua:GetSymbolIdByObjName("Symbol_null") then
			nEffectKey = nEffectKey - 1
		end	
	end		
