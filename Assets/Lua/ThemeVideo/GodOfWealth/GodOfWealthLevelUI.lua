...
function GodOfWealthLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
    self.m_transform = nil

    self.m_RegisterLoadEffectNameTable = {"TXChangeElement"}
    self.tableFixedGoSymbol = {}
end 

function GodOfWealthLevelUI:initLevelUI()
    self:InitVariable()
    GodOfWealthFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/GodOfWealth/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinAgainSplashUI = require "Lua/ThemeVideo/GodOfWealth/FreeSpinAgainSplashUI"
    self.mFreeSpinAgainSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/GodOfWealth/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

end       

function GodOfWealthLevelUI:Update()

end 

function GodOfWealthLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(GodOfWealthFunc)
end

function GodOfWealthLevelUI:CancelLeanTween()
...
function GodOfWealthLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function GodOfWealthLevelUI:FindSymbolElement(goSymbol, strKey)
    local tablePoolKey = {"gem1"}
...
function GodOfWealthLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if GodOfWealthSymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not GodOfWealthFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    GodOfWealthFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function GodOfWealthLevelUI:isScatterPossibleUseful(nReelId)
    ...
    if SlotsGameLua.m_GameResult:InFreeSpin() then
        return nScatterCount + SlotsGameLua.m_nReelCount - 1 - nReelId >= 2
    else
        return nScatterCount + SlotsGameLua.m_nReelCount - nReelId >= 3
    end
end

function GodOfWealthLevelUI:isNeedWaitingFreeSpin(nReelId) 
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
function GodOfWealthLevelUI:CheckRecoverInfo()
    self.tableCachePos = {}
...
function GodOfWealthLevelUI:ShowScatterBonusEffect()
...
function GodOfWealthLevelUI:handleFreeSpinBegin()
...
function GodOfWealthLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function GodOfWealthLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function GodOfWealthLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function GodOfWealthLevelUI:PlayFreeSpinBeginHideAni()
...
function GodOfWealthLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function GodOfWealthLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function GodOfWealthLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function GodOfWealthLevelUI:CacheSymbolElementLayer(goSymbol)
...
function GodOfWealthLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function GodOfWealthLevelUI:GetEffectByEffectPool(strKey)
...
function GodOfWealthLevelUI:RecycleEffectToEffectPool(UsedObj)
