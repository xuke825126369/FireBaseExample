...
function FarmLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
...
function FarmLevelUI:initLevelUI()
    self:InitVariable()
    FarmFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/Farm/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinAgainSplashUI = require "Lua/ThemeVideo/Farm/FreeSpinAgainSplashUI"
    self.mFreeSpinAgainSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/Farm/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

end     

function FarmLevelUI:Update()

end 

function FarmLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(FarmFunc)
end

function FarmLevelUI:CancelLeanTween()
...
function FarmLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function FarmLevelUI:FindSymbolElement(goSymbol, strKey)
...
function FarmLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if FarmSymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not FarmFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    FarmFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function FarmLevelUI:isScatterPossibleUseful(nReelId)
...
function FarmLevelUI:isNeedWaitingFreeSpin(nReelId)
...
function FarmLevelUI:CheckRecoverInfo()
    self.tableCachePos = {}
...
function FarmLevelUI:ShowScatterBonusEffect()
...
function FarmLevelUI:handleFreeSpinBegin()
...
function FarmLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function FarmLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function FarmLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function FarmLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function FarmLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function FarmLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function FarmLevelUI:CacheSymbolElementLayer(goSymbol)
...
function FarmLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function FarmLevelUI:GetEffectByEffectPool(strKey)
...
function FarmLevelUI:RecycleEffectToEffectPool(UsedObj)
