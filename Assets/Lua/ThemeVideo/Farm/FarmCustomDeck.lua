...
function FarmCustomDeck:ModifyDeckForTriggerFreeSpin(deck)
    ...
    local bTrigger, nTriggerCount = FarmConfig:orTriggerFreeSpin()
    if bTrigger then
        for x = 0, SlotsGameLua.m_nReelCount - 1 do
            for y = 0, SlotsGameLua.m_nRowCount - 1 do
                ...
                if FarmSymbol:isScatterSymbol(nSymbolId) then
                    deck[nKey] = FarmSymbol:GetCommonSymbolId(nKey)
                end
            end
        end 
        ...
        if FarmFunc:bCanTriggerFreeSpin(deck) then
            local tempDeck = {}
