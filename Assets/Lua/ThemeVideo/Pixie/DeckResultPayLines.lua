...
function DeckResultPayLines:PlayHitLineEffect2(effectPos, nEffectKey, x, y)
	...
	if not bHasHitEffectFlag then
		...
		local effectObj = PixieLevelUI:GetEffectByEffectPool("lztukuai")
		effectObj.transform:SetParent(PixieLevelUI.m_transform, false)
...
function DeckResultPayLines:checkNeedReusedHitLineEffect()
	...
	for i=1, cnt do
		...
		PixieLevelUI:RecycleEffectToEffectPool(effectObj)
...
function DeckResultPayLines:MatchLineHide()
	...
	for k, v in pairs(self.m_mapHitLineEffect) do
		PixieLevelUI:RecycleEffectToEffectPool(v)
	end
	self.m_mapHitLineEffect = {}
...
function DeckResultPayLines:GetGoSymbol(nReelId, nRowIndex)
	...
	if PixieFunc.tableBigSymbol[nReelId] then
		local nTargetRowIndex = PixieFunc.tableBigSymbol[nReelId][1]
		if nRowIndex <= nTargetRowIndex and nRowIndex >= nTargetRowIndex - 3 then
