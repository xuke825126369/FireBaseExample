...
function IndiaLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
...
function IndiaLevelUI:initLevelUI()
    self:InitVariable()
    IndiaFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/India/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/India/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

end       

function IndiaLevelUI:Update()

end 

function IndiaLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(IndiaFunc)
end

function IndiaLevelUI:CancelLeanTween()
...
function IndiaLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function IndiaLevelUI:FindSymbolElement(goSymbol, strKey)
...
function IndiaLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if IndiaSymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not IndiaFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    IndiaFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function IndiaLevelUI:isScatterPossibleUseful(nReelId)
...
function IndiaLevelUI:isNeedWaitingFreeSpin(nReelId) 
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
function IndiaLevelUI:CheckRecoverInfo()
    self.tableCachePos = {}
...
function IndiaLevelUI:ShowScatterBonusEffect()
...
function IndiaLevelUI:handleFreeSpinBegin()
...
function IndiaLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function IndiaLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function IndiaLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function IndiaLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function IndiaLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function IndiaLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function IndiaLevelUI:CacheSymbolElementLayer(goSymbol)
...
function IndiaLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function IndiaLevelUI:GetEffectByEffectPool(strKey)
...
function IndiaLevelUI:RecycleEffectToEffectPool(UsedObj)
