...
function IndiaCustomDeck:ModifyDeckForTriggerFreeSpin(deck)
    local rt = SlotsGameLua.m_GameResult
    if IndiaFunc.m_bSimulationFlag then
        ...
        for x = 0, SlotsGameLua.m_nReelCount - 1 do
            for y = 0, SlotsGameLua.m_nRowCount - 1 do
                ...
                if IndiaSymbol:isScatterSymbol(nSymbolId) then
                    nSymbolId = IndiaSymbol:GetCommonSymbolId(nKey)
                end
                
                if IndiaSymbol:isCanDoubleSymbol(nSymbolId) then
                    nSymbolId = IndiaFunc:GetCanDoubleSymbolId(nSymbolId)
                end
    ...
    local bTrigger, nTriggerCount = IndiaConfig:orTriggerFreeSpin()
    if bTrigger then
        for x = 0, SlotsGameLua.m_nReelCount - 1 do
            for y = 0, SlotsGameLua.m_nRowCount - 1 do
                local nKey = SlotsGameLua.m_nRowCount * x + y
                local nSymbolId = deck[nKey]
                if IndiaSymbol:isScatterSymbol(nSymbolId) then
                    deck[nKey] = IndiaSymbol:GetCommonSymbolId(nKey)
                end
            end
        end 
        ...
        if IndiaFunc:bCanTriggerFreeSpin(deck) then
            local tempDeck = {}
