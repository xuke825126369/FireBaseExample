...
function JackPotUI:InitVariable()
	...
	self.tableJackPotAddSumMoneyCount = {0} -- 累积的JackPot 增加值
end
...
function JackPotUI:Init()
	...
	self.tableJackPotText = {}
	self.tableJackPotText[1] = self.m_transform:FindDeepChild("CollectInfo/TextMeshProJackpotValue"):GetComponent(typeof(TextMeshPro))
...
function JackPotUI:addJackPotValue()
	...
	local nJackPotCount = #ThreePigsConfig.TABLE_JACKPOT_ADDCOEF
	for i = 1, nJackPotCount do
		self.tableJackPotAddSumMoneyCount[i] = self.tableJackPotAddSumMoneyCount[i] + nTotalBet * ThreePigsConfig.TABLE_JACKPOT_ADDCOEF[i]
	end

	if not ThreePigsFunc.m_bSimulationFlag then
		for i = 1, nJackPotCount do
			local fValue = self:GetTotalJackPotValue(i)
...
function JackPotUI:GetTotalJackPotValue(i)
	return SceneSlotGame.m_nTotalBet * ThreePigsConfig.TABLE_JACKPOT_MIN_MULTUILE[i] + self.tableJackPotAddSumMoneyCount[i]
end
