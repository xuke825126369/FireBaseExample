...
function ThreePigsLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
    self.m_transform = nil

    self.m_RegisterLoadEffectNameTable = {"CollectPigEffect", "preCollectEffect"}
    self.tableFixedGoSymbol = {}
    self.tableCollectEffect = {}
end 

function ThreePigsLevelUI:initLevelUI()
    self:InitVariable()
    ThreePigsFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/ThreePigs/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinAgainSplashUI = require "Lua/ThemeVideo/ThreePigs/FreeSpinAgainSplashUI"
    self.mFreeSpinAgainSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/ThreePigs/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

    self.mJackPotUI = require "Lua/ThemeVideo/ThreePigs/JackPotUI"
    self.mJackPotUI:Init()

    self.mJackPotSplashUI = require "Lua/ThemeVideo/ThreePigs/JackPotSplashUI"
    self.mJackPotSplashUI:Init()

    self.textCollectCount = self.m_transform:FindDeepChild("CollectInfo/TextMeshProCollectInfo"):GetComponent(typeof(TextMeshPro))
    self.goLinePrefab = self.m_transform:FindDeepChild("preLineEffectCanvas/LinesDir/lineImage").gameObject
    self.goLinePrefab:SetActive(false)

    self.goCollectInfo = self.m_transform:FindDeepChild("CollectInfo").gameObject
    self.goCollectInfo:SetActive(true)
end       

function ThreePigsLevelUI:Update()
    self.mJackPotUI:Update()
end 

function ThreePigsLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(ThreePigsFunc)
end

function ThreePigsLevelUI:CancelLeanTween()
...
function ThreePigsLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function ThreePigsLevelUI:FindSymbolElement(goSymbol, strKey)
...
function ThreePigsLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if ThreePigsSymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not ThreePigsFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    ThreePigsFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function ThreePigsLevelUI:isScatterPossibleUseful(nReelId)
...
function ThreePigsLevelUI:isNeedWaitingFreeSpin(nReelId)
...
function ThreePigsLevelUI:CheckRecoverInfo()
    ThreePigsLevelUI:getDBCollectCount()
...
function ThreePigsLevelUI:UpdateCollectInfo()
    self.textCollectCount.text = ThreePigsFunc.nCollectCount.."/"..ThreePigsConfig.N_COLLECT_MAX_COUNT
end

-- 显示Scatter 特效 -- 统一使用播放spine动画的接口: PlayActiveAnimation(enumType, true, 1.0)
function ThreePigsLevelUI:ShowScatterBonusEffect()
...
function ThreePigsLevelUI:handleFreeSpinBegin()
...
function ThreePigsLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function ThreePigsLevelUI:ShowCustomBigMoneySplash()
    self.mJackPotSplashUI:Show()
end

function ThreePigsLevelUI:PlayFreeSpinBeginHideAni()
    self.goCollectInfo:SetActive(false)
end

function ThreePigsLevelUI:PlayFreeSpinFinishHideAni()
    self.goCollectInfo:SetActive(true)
end

function ThreePigsLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function ThreePigsLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function ThreePigsLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function ThreePigsLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function ThreePigsLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function ThreePigsLevelUI:CacheSymbolElementLayer(goSymbol)
...
function ThreePigsLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function ThreePigsLevelUI:GetEffectByEffectPool(strKey)
...
function ThreePigsLevelUI:RecycleEffectToEffectPool(UsedObj)
...
-------------------------------------------- ReSpin 记录 -----------------------------------------------
function ThreePigsLevelUI:setDBCollectCount()
	if LevelDataHandler.m_Data.mThemeData == nil then
		LevelDataHandler.m_Data.mThemeData = {}
    end

    LevelDataHandler.m_Data.mThemeData.nCollectCount = ThreePigsFunc.nCollectCount
    LevelDataHandler.m_Data.mThemeData.nSelectFreeSpinType = ThreePigsFunc.nSelectFreeSpinType
    LevelDataHandler:persistentData()
end

function ThreePigsLevelUI:getDBCollectCount()
	if LevelDataHandler.m_Data.mThemeData == nil then
		return
	end

	if LevelDataHandler.m_Data.mThemeData.nCollectCount == nil then
		return
	end         

    ThreePigsFunc.nCollectCount = LevelDataHandler.m_Data.mThemeData.nCollectCount
    ThreePigsFunc.nSelectFreeSpinType = LevelDataHandler.m_Data.mThemeData.nSelectFreeSpinType
end
