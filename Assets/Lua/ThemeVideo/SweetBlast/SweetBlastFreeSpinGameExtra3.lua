SweetBlastFreeSpinGameExtra3 = {} -- 右下的棋盘
SweetBlastFreeSpinGameExtra3.m_transform = nil -- "X4_4"

SweetBlastFreeSpinGameExtra3.m_listReelLua = {} -- 5个 MultiSlotsReel 

SweetBlastFreeSpinGameExtra3.m_fSymbolHeight = 0
SweetBlastFreeSpinGameExtra3.m_fDampingHeight = 100
SweetBlastFreeSpinGameExtra3.m_nRowCount = 3
SweetBlastFreeSpinGameExtra3.m_nReelCount = 5
SweetBlastFreeSpinGameExtra3.m_listDeck = {} -- 从 0 到 14 的 15 SymbolID
SweetBlastFreeSpinGameExtra3.m_fCentBoardY = 0

SweetBlastFreeSpinGameExtra3.m_fSpeedMax = 0
SweetBlastFreeSpinGameExtra3.m_fRotateDistance = 2000

SweetBlastFreeSpinGameExtra3.m_nReelsType = -1 -- SweetBlastReelsType.ReelsTypeNull -- 运行时修改设置

SweetBlastFreeSpinGameExtra3.m_nActiveReel = -1
SweetBlastFreeSpinGameExtra3.m_fSpinAge = 0

SweetBlastFreeSpinGameExtra3.m_GameResult = {} -- 记录中奖信息 展示线时候需要用到

SweetBlastFreeSpinGameExtra3.m_bAllReelStop = true

SweetBlastFreeSpinGameExtra3.m_listStickySymbol = {} --固定的元素
SweetBlastFreeSpinGameExtra3.m_trReels = {nil, nil, nil, nil, nil}

SweetBlastFreeSpinGameExtra3.m_listHitSymbols = {}

SweetBlastFreeSpinGameExtra3.m_mapSymbolPool = {} -- 这个棋盘上要用的元素都来这里取 用完都放回这里
-- prfab : listGo
-- 放在这个节点下的元素就和棋盘上的元素是同一个group了 在加入棋盘的时候就不用build了
SweetBlastFreeSpinGameExtra3.m_trSymbolsPool = nil --SymbolsPool4X_4


function SweetBlastFreeSpinGameExtra3:Start()
end

function SweetBlastFreeSpinGameExtra3:OnDisable()
end

function SweetBlastFreeSpinGameExtra3:OnDestroy()
end

function SweetBlastFreeSpinGameExtra3:getSymbolObject(nSymbolID)
    local go = SweetBlastFreeSpinCommon:getSymbolObject(nSymbolID, SweetBlastFreeSpinGameExtra3)
    return go
end

function SweetBlastFreeSpinGameExtra3:reuseSymbolObject(go)
    SweetBlastFreeSpinCommon:reuseSymbolObject(go, SweetBlastFreeSpinGameExtra3)
end

function SweetBlastFreeSpinGameExtra3:init()
    ...
    SweetBlastFreeSpinCommon:initSymbolPool(SweetBlastFreeSpinGameExtra3)

    self:initLevelParam()
end

function SweetBlastFreeSpinGameExtra3:initReelsTransform()
...
function SweetBlastFreeSpinGameExtra3:getReelsTransformAndTrPool(trFreeSpinNode)
    local nFreeSpinType = SweetBlastFreeSpinCommon.m_nFreeSpinType
    ...
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin3X5_4 then
        strGroupName = "Group4_4"
        strPoolName = "SymbolsPool3X5_4_4"
    ...
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin4X5_4 then
        strGroupName = "Group4_4"
        strPoolName = "SymbolsPool4X5_4_4"
    end
...
function SweetBlastFreeSpinGameExtra3:getReelsType() -- 第四个棋盘
    local nReelsType = 0
    ...
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin3X5_4 then
        nReelsType = SweetBlastReelsType.ReelsType3X5_4_4
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin4X5_2 then
        
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin4X5_3 then
        
    elseif nFreeSpinType == EnumSweetBlastFreeSpinType.FreeSpin4X5_4 then
        nReelsType = SweetBlastReelsType.ReelsType4X5_4_4
    end
    
    return nReelsType
end

function SweetBlastFreeSpinGameExtra3:initLevelParam()
    self.m_GameResult = GameResult:create(SweetBlastFreeSpinGameExtra3)
    ...
    for i=0, reelCount-1 do
        local reelLua = MultiSlotsReel:create(i, nReelRow, SweetBlastFreeSpinGameExtra3)
    ...
    SweetBlastFreeSpinCommon:CreateReelRandomSymbolList(SweetBlastFreeSpinGameExtra3)
...
function SweetBlastFreeSpinGameExtra3:SetRandomSymbolToReel()
...
function SweetBlastFreeSpinGameExtra3:RepositionSymbols()
...
function SweetBlastFreeSpinGameExtra3:OnPreReelStop(nReelID) -- 0 1 2 3 4
...
function SweetBlastFreeSpinGameExtra3:getDeck()
...
function SweetBlastFreeSpinGameExtra3:ModifyTestDeck(deck) -- 调试的时候自己修改数据用
...
function SweetBlastFreeSpinGameExtra3:Update()
    ...
    if self.m_nActiveReel == -1 then
        ...
        if self.m_fSpinAge > 0.5 then -- spin开始后0.5s可以允许停
         ...
         --   Debug.Log("----- SweetBlastFreeSpinGameExtra3:Update() ----- getDeck -----")
        end
    else
...
function SweetBlastFreeSpinGameExtra3:PreCheckWin()
    self:CheckWinEnd()
end

function SweetBlastFreeSpinGameExtra3:CheckWinEnd()
    ...
    local fGameExtra3Win = self.m_GameResult.m_fSpinWin
    local strTemp = MoneyFormatHelper.coinCountOmit(fGameExtra3Win)
 --   Debug.Log("----SweetBlastFreeSpinGameExtra3:CheckWinEnd()----fGameExtra3Win: " .. strTemp)

    self:CheckWinEndPost()
end

function SweetBlastFreeSpinGameExtra3:Spin() -- 点spin按钮时候会调用
...
function SweetBlastFreeSpinGameExtra3:OnStartSpin()
    -- 开始滚动了 比如要隐藏spine动画的。。。
...
function SweetBlastFreeSpinGameExtra3:OnSpinEnd()
    self:showSpineFrame0(false)
end

function SweetBlastFreeSpinGameExtra3:CheckWinEndPost()
...
function SweetBlastFreeSpinGameExtra3:ShowSpinResult()
end

function SweetBlastFreeSpinGameExtra3:playWinEffect()
...
function SweetBlastFreeSpinGameExtra3:ShowPayLineEffect(nLineID, nMaxMatchReelID)
...
function SweetBlastFreeSpinGameExtra3:PlayHitLineEffect(x, y)
    local trParent = self.m_listReelLua[x].m_listGoSymbol[y].transform
    
    local nEffectKey = self.m_nRowCount * x + y + 300 -- 棋盘4的就在此基础上加300
...
function SweetBlastFreeSpinGameExtra3:PlaySpineEffect(x, y)
	local go = nil
    local nEffectKey = self.m_nRowCount * x + y + 300
...
function SweetBlastFreeSpinGameExtra3:PlayMultiClipEffect(x, y)
	local go = nil
    local nEffectKey = self.m_nRowCount * x + y + 300
    if PayLinePayWaysEffectHandler.m_mapSpineEffects[nEffectKey] ~= nil then
        return -- 已经在播放了
...
function SweetBlastFreeSpinGameExtra3:LoopScaleSymbol(x, y)
	local nEffectKey = self.m_nRowCount * x + y + 300
...
function SweetBlastFreeSpinGameExtra3:showSpineFrame0(bShowFrame0)
...
function SweetBlastFreeSpinGameExtra3:showSpineNode() -- 滚动停止的时候调用
