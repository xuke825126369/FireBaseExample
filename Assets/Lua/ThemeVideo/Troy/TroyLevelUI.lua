...
function TroyLevelUI:InitVariable()
    self.m_LeanTweenIDs = {}
    self.m_transform = nil
    
    self.m_RegisterLoadEffectNameTable = {"lztukuai"}
    self.tableFixedGoSymbol = {}
end 

function TroyLevelUI:initLevelUI()
    self:InitVariable()
    TroyFunc:InitVariable()
    ...
    self.mFreeSpinBeginSplashUI = require "Lua/ThemeVideo/Troy/FreeSpinBeginSplashUI"
    self.mFreeSpinBeginSplashUI:Init()

    self.mFreeSpinAgainSplashUI = require "Lua/ThemeVideo/Troy/FreeSpinAgainSplashUI"
    self.mFreeSpinAgainSplashUI:Init()

    self.mFreeSpinFinishSplashUI = require "Lua/ThemeVideo/Troy/FreeSpinFinishSplashUI"
    self.mFreeSpinFinishSplashUI:Init()

    self.mDeckResultPayLines = require "Lua/ThemeVideo/Troy/DeckResultPayLines"
    self.mDeckResultPayLines:Init(SlotsGameLua)

end     

function TroyLevelUI:Update()
    self.mDeckResultPayLines:ShowAllMatchLines()
    self.mDeckResultPayLines:DisplayMatchLinesInfo()
end 

function TroyLevelUI:OnDestroy()
    self:CancelLeanTween()
    LuaHelper.ReleaseVariable(self)
    LuaHelper.ReleaseVariable(TroyFunc)
end

function TroyLevelUI:CancelLeanTween()
...
function TroyLevelUI:SetInteger(mAnimator, strKey, nAction)
...
function TroyLevelUI:FindSymbolElement(goSymbol, strKey)
...
function TroyLevelUI:OnPreReelStop(nReelId)
    ...
    if self:isScatterPossibleUseful(nReelId) then
        for i = 0, SlotsGameLua.m_nRowCount - 1 do
            ...
            if TroySymbol:isScatterSymbol(nSymbolId) then
                ...
                elseif not TroyFunc.m_bScatterSound then
                    AudioHandler:PlayScatterStopSound(nReelId)
                    TroyFunc.m_bScatterSound = true
                end
            end
        end
    end
...
function TroyLevelUI:isScatterPossibleUseful(nReelId)
...
function TroyLevelUI:isNeedWaitingFreeSpin(nReelId)
...
function TroyLevelUI:CheckRecoverInfo()
    self.tableCachePos = {}
...
function TroyLevelUI:ShowScatterBonusEffect()
...
function TroyLevelUI:handleFreeSpinBegin()
...
function TroyLevelUI:handleFreeSpinEnd()
    PayLinePayWaysEffectHandler:MatchLineHide()
    self.mFreeSpinFinishSplashUI:Show()
end

function TroyLevelUI:FillSymbol(nSymbolId, nReelId, nRowIndex)
...
function TroyLevelUI:FixedAliceSymbol(nSymbolId, nReelId, nRowIndex)
...
function TroyLevelUI:ResetTotalWinToUI(fMoneyCount)
    SlotsGameLua.m_GameResult.m_fGameWin = fMoneyCount
    SceneSlotGame.m_SlotsNumberWins:End(fMoneyCount)
end

function TroyLevelUI:UpdateTotalWinToUI(fAddMoneyCount, fAniTime)
...
function TroyLevelUI:CollectMoneyToDB(fAddMoneyCount)
    if SlotsGameLua.m_GameResult:InFreeSpin() then
...
---------------------- 设置 符号 的层级 -----------------------------
function TroyLevelUI:CacheSymbolElementLayer(goSymbol)
...
function TroyLevelUI:SetModifyLayer(goSymbol, nOrder)
...
function TroyLevelUI:GetEffectByEffectPool(strKey)
...
function TroyLevelUI:RecycleEffectToEffectPool(UsedObj)
