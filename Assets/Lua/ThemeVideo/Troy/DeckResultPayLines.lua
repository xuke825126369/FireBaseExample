...
function DeckResultPayLines:PlayHitLineEffect2(effectPos, nEffectKey, x, y)
	...
	if not bHasHitEffectFlag then
		local strEffectName = "lztukuai"
		local effectObj = TroyLevelUI:GetEffectByEffectPool(strEffectName)
		effectObj.transform:SetParent(TroyLevelUI.m_transform, false)
...
function DeckResultPayLines:checkNeedReusedHitLineEffect()
	...
	for i=1, cnt do
		...
		TroyLevelUI:RecycleEffectToEffectPool(effectObj)
...
function DeckResultPayLines:MatchLineHide()
	...
	for k, v in pairs(self.m_mapHitLineEffect) do
		TroyLevelUI:RecycleEffectToEffectPool(v)
	end
	self.m_mapHitLineEffect = {}
...
function DeckResultPayLines:GetGoSymbol(nReelId, nRowIndex)
	...
	if nSymbolId == SlotsGameLua:GetSymbolIdByObjName("NormalWild_1") then
		nEffectKey = nReelId * self.m_Manager.m_nRowCount + 1
		goSymbol =  self.m_Manager.m_listReelLua[nReelId].m_listGoSymbol[1]
	end			
