...
StarPop.gameObject = nil
StarPop.transform = nil
StarPop.fullLoadingGameObject = nil

local yield_return = (require 'cs_coroutine').yield_return

function StarPop:isActiveShow()
    return self.gameObject and self.gameObject.activeInHierarchy
end

function StarPop:createAndShow(bIsInQueue)
    local endTime, ratio, strType = SaleAdHandler:checkNormalSalesType("StarPop")
    if strType == nil or strType ~= "StarPop" then
        return
    end
    if DBHandler:checkHasSpecialSaleList("StarPop") then
        return
    end
    if self.gameObject == nil then
        self.gameObject = Unity.Object.Instantiate(Util.getHotPrefab("Assets/BaseHotAdd/RocketsSalesShop/StarPop.prefab"))
...
function StarPop:refreshContent(ratio)
    ...
    local skuInfo = SaleAdHandler:getSalesSkuInfo(productId, ratio, "StarPop")
...
function StarPop:OnEnable()
    NotificationHandler:addObserver(self, "onPurchaseDoneNotifycation")
    NotificationHandler:addObserver(self, "onPurchaseFailedNotifycation")
end

function StarPop:OnDisable()
    NotificationHandler:removeObserver(self)
end

function StarPop:onCloseBtnClicked()
...
function StarPop:onPurchaseDoneNotifycation()
    self:hideLoading()
    ViewScaleAni:Hide(self.transform.gameObject)
    if ThemeLoader.themeKey ~= nil then
        SlotsGameLua.m_bReelPauseFlag = false
...
function StarPop:onPurchaseFailedNotifycation()
    self:hideLoading()
    self.button.interactable = true
    CommonDialogBox:ShowSureUI("Purchase Failed")
end

function StarPop:showLoading()
	self.fullLoadingGameObject:SetActive(true)
end

function StarPop:hideLoading()
	self.fullLoadingGameObject:SetActive(false)
end

function StarPop:getShopSkus()
    local productId = "com.slots.goldfever.coins001"
    local reviewList = {"com.slots.goldfever.coins005",
...
function StarPop:updateLeftTime(endTime)
...
-- function StarPop:setCardsContainer(parent, skuInfo)
