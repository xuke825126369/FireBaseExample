...
namespace BookCurlPro
{
    ...
    public class BookPro : MonoBehaviour
    {
        ...
        public RectTransform LeftHotSpotTransform;
        public RectTransform RightHotSpotTransform;

        public bool stopDraginPaperFlag = false;
        ...
        public void SetCurrentPaper(int paperIndex)
        {
            if(paperIndex != currentPaper)
            {
                if (paperIndex < StartFlippingPaper)
                    currentPaper = StartFlippingPaper;
                else if (paperIndex > EndFlippingPaper + 1)
                    currentPaper = EndFlippingPaper + 1;
                else
        ...
        bool isBeginInLeft = false;
        bool isBeginInRight = false;
        // Update is called once per frame
        void Update()
        {
            if (stopDraginPaperFlag && !pageDragging)
                return;
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                bool bLeftFlag = RectTransformUtility.RectangleContainsScreenPoint(LeftHotSpotTransform, screePoint, main);
                bool bRightFlag = RectTransformUtility.RectangleContainsScreenPoint(RightHotSpotTransform, screePoint, main);
                if (bLeftFlag)
                {
                    isBeginInLeft = true;
                }
            ...
            if (Input.GetMouseButton(0) && !pageDragging)
            {
                var screePoint = (Vector2)Input.mousePosition;
                bool bLeftFlag = RectTransformUtility.RectangleContainsScreenPoint(LeftHotSpotTransform, screePoint, main);
                bool bRightFlag = RectTransformUtility.RectangleContainsScreenPoint(RightHotSpotTransform, screePoint, main);
                ...
                {
                    if ((bLeftFlag && isBeginInLeft) || (bRightFlag && isBeginInRight))
                    {
                        if (!isBegin)
                        {
                            firstTouchPos = screePoint;
                            isBegin = true;
                        }
                        else
                        {
                            var distance = Vector2.Distance(screePoint, firstTouchPos);
                            if (distance >= minDis)
                            {

                                Vector3 mouseLocalPos = transformPointMousePosition(Input.mousePosition);
                                if (bLeftFlag)
                                {
                                    DragLeftPageToPoint(mouseLocalPos);
                                }
                                else if (bRightFlag)
                                {
                                    DragRightPageToPoint(mouseLocalPos);
                                }
                            }
                        }
                    }
