        ...
        CGPROGRAM
            ...
            v2f vert(appdata_t v)
            {
				...
				for(int i = -1; i <= 1; i++)
				{
					for(int j = -1; j <= 1; j++)
					{
						OUT.uv[k] = uv + float2(_MainTex_TexelSize.x * i ,  _MainTex_TexelSize.y * j) * float2(_BlurSizeX, _BlurSizeY);
						k++;
					}
				}
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                half4 color = IN.color * (averageColor + _TextureSampleAdd);
